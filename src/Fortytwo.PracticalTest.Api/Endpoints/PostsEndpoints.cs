...
namespace Fortytwo.PracticalTest.Api.Endpoints
{
    public static class PostsEndpoints
    {
        ...
        {
            app.MapGet("/posts", [Authorize] async (string? TitleOrBodySearchTerm, int? CreatedByUserId, int? PageNumber, int? PageSize, IGetPostsQueryHandler getPostsQueryHandler, CancellationToken token, ILogger<Program> logger) =>
            {
            ...
            app.MapGet("/posts/{id}", [Authorize] async (int id, IGetPostByIdQueryHandler getPostByIdQueryHandler, CancellationToken token, ILogger<Program> logger) =>
            {
